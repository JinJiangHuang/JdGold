...
public class Util {
	 ...
	 * 和获取最新的金价
	 *
	 * json格式： { "resultCode": 0, "resultMsg": "操作成功", "resultData": { "datas": {
	 * "productSku": "P005", "demode": false, "priceNum":
	 * "8B2AE5F3F7DDD73555B62B590E96233A", "price": "347.17", "id": 5503879, "time":
	 * "1565864548000" }, "status": "SUCCESS"* }, "channelEncrypt": 0 }
	 *
	 * @return 最新价格
	 */
	...
	public static JSONObject getLastSecondData(JSONArray jsonArray, int seconds) {
		...
		return jsonArray.stream().limit(seconds / 30 + 10).map(s -> (JSONObject) s)
				.filter(s -> getTime(s) + (seconds * 1000) <= timeCurrent).findFirst().orElse(current);
	}
	...
	public static String getNowExamInfo(String reqUrl) {
		try {
...
//				Collections.reverse(jsonArray);
	...
	public static void main(String[] args) {
		String reqUrl=String.format(Config.EXAMQZ, Config.EXAMID2);
		System.out.println(getNowExamInfo(reqUrl));
	}
}
